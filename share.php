...
if($reflinkshare != "")
{
    if($prelink != "".$homedir."/share.php")
    {
        if($prelink != $reflinkshare)
        {
...
<br /><br />
                    ...
                    <div class="reactions">
                        <h3 style='font-size : 40px;'>Auto Share Facebook</h3>
...
if(isset($_POST['auto-submit']))
{
    ...
    $update_autoshare = "UPDATE FROM autoshare SET name=N'$name',auto_delay=$auto_delay,auto_date=now() WHERE userid='$userid'";
    $insert_autoshare = "INSERT INTO autoshare(userid,name,token,auto_delay,auto_date) VALUES('$userid',N'$name','$tokenn','$auto_delay',now())";
    $select_autoshare = "SELECT * FROM autoshare WHERE userid='$userid'";
    $check_aut = getme($tokenn);
    $data_token = mysqli_fetch_array(mysqli_query($conn, $select_autoshare));
    if($check_aut)
    {
        ...
        {
            if(mysqli_num_rows(mysqli_query($conn, $select_autoshare))>=1)
            {
                if($data_token['auto_delay']>= time())
                {
                    ...
                    mysqli_query($conn,$update_autoshare);
                    action($post,$query_num,$count_delay,$conn);
                }
            }
            else
            {
                mysqli_query($conn,$insert_autoshare);
                action($post,$query_num,$count_delay,$conn);
...
function action($post,$query_num,$count_delay,$conn)
{
    ...
    echo "<script>alert('Share thành công :D, vui lòng chờ ".$count_delay." phút để dùng tiếp');top.location.href='".$homedir."/user.php';</script>";
}
?>