...
if($reflinkfollow != "")
{
    if($prelink != "".$homedir."/follow.php")
    {
        if($prelink != $reflinkfollow)
        {
...
<br /><br />
                    ...
                    <div class="reactions">
                        <h3 style='font-size : 40px;'>Auto Follow Facebook - Tăng Người Theo Dõi Facebook</h3>
...
<h4>ID của tài khoản cần tăng Follow</h4>

                        <input class='input-content' required name='postid' type="text" placeholder='Nhập ID tài khoản cần tăng follows' > </input>
...
if(isset($_POST['auto-submit']))
{
    ...
    $update_autofollow = "UPDATE FROM autofollow SET name=N'$name',auto_delay=$auto_delay,auto_date=now() WHERE userid='$userid'";
    $insert_autofollow = "INSERT INTO autofollow(userid,name,token,auto_delay,auto_date) VALUES('$userid',N'$name','$tokenn','$auto_delay',now())";
    $select_autofollow = "SELECT * FROM autofollow WHERE userid='$userid'";
    $check_aut = getme($tokenn);
    $data_token = mysqli_fetch_array(mysqli_query($conn, $select_autofollow));
    if($check_aut)
    {
        ...
        {
            if(mysqli_num_rows(mysqli_query($conn, $select_autofollow))>=1)
            {
                if($data_token['auto_delay']>= time())
                {
                    ...
                    mysqli_query($conn,$update_autofollow);
                    action($post,$query_num,$count_delay,$conn);
                }
            }
            else
            {
                mysqli_query($conn,$insert_autofollow);
                action($post,$query_num,$count_delay,$conn);
...
function action($post,$query_num,$count_delay,$conn)
{
    $son = mysqli_query($conn,$query_num);
    while($gettk = mysqli_fetch_array($son))
    {
        $full = trim($gettk['token']);
        auto("https://graph.facebook.com/".$post."/subscribers?method=post&access_token=".$full);
    }

    echo "<script>alert('Auto Follow thành công :D, vui lòng chờ ".$count_delay." phút để dùng tiếp');top.location.href='".$homedir."/user.php';</script>";
}
?>